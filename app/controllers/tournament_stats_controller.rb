class TournamentStatsController < ApplicationController
  ...
  def show
    ...
    @revenue_players = (@tournament.total_player_tickets - @player_tickets_left) * @tournament.player_price
    @revenue_spectators = (@tournament.total_audience_tickets - @spectator_tickets_left) * @tournament.spectator_price
    @revenue_dinner = (@tournament.total_dinner_tickets - @dinner_tickets_left) * @tournament.dinner_price
  	@revenue_sponsors = 0
    TournamentSponsorship.where(tournament_id: @id).find_each do |sponsor|
  	  @revenue_sponsors += sponsor.ticket_price
  	end
    @revenue = @revenue_players + @revenue_spectators + @revenue_dinner + @revenue_sponsors
